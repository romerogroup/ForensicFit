...
def learner(array): # 4 x 2284x420 x 1 each image  
        #encoder 
        #in put = 2284x420
        ...
        conv1_1 = Conv2D(16, (3,3), activation='relu', padding='same')(img1) # 2284x420 x 16 
        pool1_1 = MaxPooling2D(pool_size=(2,2))(conv1_1) # 1142x210x16
        conv2_1 = Conv2D(8, (3, 3), activation='relu', padding='same')(pool1_1) # 1142x210 x 8
        pool2_1 = MaxPooling2D(pool_size=(2,2))(conv2_1) # 571 x 105 x 8
        conv3_1 = Conv2D(8, ( 3,3), activation='relu', padding='same')(pool2_1) # 571x105 x 8
        pool3_1 = MaxPooling2D(pool_size=(2,2))(conv3_1) # 285x 52 x 8
        conv4_1 = Conv2D(8, ( 3,3), activation='relu', padding='same')(pool3_1) # 285x52x8
        pool4_1 = MaxPooling2D(pool_size=(2,2))(conv4_1) # 142x 26x8
        conv5_1 = Conv2D(8, ( 3,3), activation='relu', padding='same')(pool4_1) # 142x26x8
        pool5_1 = MaxPooling2D(pool_size=(2,2))(conv5_1) # 71 x 13 x8
        conv6_1 = Conv2D(8, ( 3,3), activation='relu', padding='same')(pool5_1) # 71x13x8
        pool6_1 = MaxPooling2D(pool_size=(2,2))(conv6_1) # 35 x4x8
        flat_1 = Flatten()(pool6_1)  # 1120x 1 
        ...
        layer1 = Concatenate()([flat_1,flat_2,flat_3,flat_4]) # 4480
...
data = data.reshape(-1,4,2284,420,1)
#data = data/255.
batch_size = 20 
epochs = 50 
inChannel = 1 
input_img = Input(shape = (4,2284,420, inChannel)) 
model = Model(input_img, learner(input_img)) 
model.compile(optimizer=RMSprop(),loss='binary_crossentropy',metrics=['accuracy'])
#model.compile(loss='mean_squared_error', optimizer = RMSprop()) 
model.summary() 

train_X,test_X,train_y,test_y = train_test_split(data, matches,test_size=0.2, random_state=13)
