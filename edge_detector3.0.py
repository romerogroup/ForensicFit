...
#        cv2.imwrite("mask%i.tif"%iimage,mask)
        ...
        for ic in range(len(contours)):
            if cv2.contourArea(contours[ic]) > max_con :
                ...
                max_con_arg = ic
        cv2.drawContours(im_color,contours,max_con_arg,(0,0,255),10)
#        cv2.imshow(fname+'i',cv2.resize(im_color,tuple(np.array(im_color.shape)[0:2]//3)))
#        cv2.imwrite("drawContour%i.tif"%iimage,im_color)
#        cv2.waitKey(0)
...
#        cv2.imshow(fname,edge_bw)
#        cv2.imwrite("edge_bw%i.tif"%iimage,edge_bw)
...
#            isection = cv2.copyMakeBorder(isection,3,3,3,3,cv2.BORDER_CONSTANT,value=[0,0,0])
...
#        cv2.waitKey(0)
#        cv2.imwrite("segmented%i.tif"%iimage,img_seg)
...
#        argsort = np.argsort(test[:,1])
        ...
        for ipoint in range(0,npoints):
            ...
            cond2 = edge[:,1] <= y_end
            cond_and = np.bitwise_and(cond1,cond2)
#            plt.plot(edge[cond_and][:,0],edge[edge[cond_and],1])
            data[ipoint,:] = np.average(edge[cond_and],axis=0)
#        data = boundary[cond_12]
        
#        plt.figure()
#        plt.scatter(-1*data[:,1],-1*data[:,0],s=1)
#        plt.ylim(-725,-625)
#        cv2.waitKey(0)
#        cv2.destroyAllWindows()  
#        print(edge_bw.shape)
        return data,segments,segments_4p,edge_bw
...
#    print(imatch)
    ...
    for iimage in range(4):
        data,segments,segments_4p,edge_bw = get_data(fnames[iimage])
        DATA[-1].append(data)
